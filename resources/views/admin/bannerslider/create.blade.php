            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        {{ trans('global.create') }} {{ trans('cruds.bannerslider.title_singular') }}
                    </div>
                    <div class="panel-body">
                        <form method="POST" action="{{ route("admin.manage-banner.store") }}" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group {{ $errors->has('bannerslider_img') ? 'has-error' : '' }}">
                                <label class="required" for="category_img">{{ trans('cruds.bannerslider.fields.banner_img') }}</label>
                            ...
                            <div class="form-group {{ $errors->has('bannerslider_status') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control" name="bannerslider_status" id="bannerslider_status" required>
                                    ...
                                    @foreach(App\Models\ProductCategory::CATEGORY_STATUS_SELECT as $key => $label)
                                        <option value="{{ $key }}" {{ old('bannerslider_status', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
...
@section('scripts')
    <script>
        ...
        Dropzone.options.categoryImgDropzone = {
            ...
            init: function () {
                @if(isset($BannerSlider) && $BannerSlider->bannerslider_img)
                var files = {!! json_encode($BannerSlider->bannerslider_img) !!}
                    for (var i in files) {
                    var file = files[i]
