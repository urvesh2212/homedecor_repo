        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.productvariant.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('product_variant_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="product_variant_name" id="product_variant_name" value="{{ old('product_variant_name', '') }}" required>
                        ...
                        <div class="form-group {{ $errors->has('product_variant_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="product_variant_status" id="product_variant_status" required>
                                ...
                                @foreach(App\Models\ProductVariant::PRODUCT_VARIANT_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('product_variant_status', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
