                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.sub-categories.update", [$subCategory->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('subcategory_code') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="subcategory_code" id="subcategory_code" value="{{ old('subcategory_code', $subCategory->subcategory_code) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('subcategory_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="subcategory_name" id="subcategory_name" value="{{ old('subcategory_name', $subCategory->subcategory_name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('cat') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="cat_id" id="cat_id" required>
                                @foreach($cats as $id => $cat)
                                    <option value="{{ $id }}" {{ (old('cat_id') ? old('cat_id') : $subCategory->cat->id ?? '') == $id ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('cat'))
                        ...
                        <div class="form-group {{ $errors->has('subcategory_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="subcategory_status" id="subcategory_status" required>
                                ...
                                @foreach(App\Models\SubCategory::SUBCATEGORY_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('subcategory_status', $subCategory->subcategory_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
