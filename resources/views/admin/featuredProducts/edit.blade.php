        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.featuredProduct.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.featured-products.update", [$featuredProduct->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('featured_product') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="featured_product_id" id="featured_product_id" required>
                                @foreach($featured_products as $id => $featured_product)
                                    <option value="{{ $id }}" {{ (old('featured_product_id') ? old('featured_product_id') : $featuredProduct->featured_product->id ?? '') == $id ? 'selected' : '' }}>{{ $featured_product }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('featured_product_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="featured_product_status" id="featured_product_status" required>
                                ...
                                @foreach(App\Models\FeaturedProduct::FEATURED_PRODUCT_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('featured_product_status', $featuredProduct->featured_product_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
