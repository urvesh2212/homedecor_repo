        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.offerProduct.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.offer-products.update", [$offerProduct->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('offer_product') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="offer_product_id" id="offer_product_id" required>
                                @foreach($offer_products as $id => $offer_product)
                                    <option value="{{ $id }}" {{ (old('offer_product_id') ? old('offer_product_id') : $offerProduct->offer_product->id ?? '') == $id ? 'selected' : '' }}>{{ $offer_product }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('offer_product_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="offer_product_status" id="offer_product_status" required>
                                ...
                                @foreach(App\Models\OfferProduct::OFFER_PRODUCT_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('offer_product_status', $offerProduct->offer_product_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
